...
const incomeData: Record<string, Record<string, {
  ...
  return <section className="py-16 md:py-24">
      <div className="container mx-auto px-4">
        ...
        duration: 0.6
          ...
          <span className="inline-flex items-center gap-2 text-primary px-4 py-2 rounded-full text-sm font-medium mb-4 bg-secondary">
          ...
          <div className="bg-card rounded-2xl shadow-luxury-lg border border-border overflow-hidden">
            ...
            <div className="p-6 md:p-8 border-b border-border bg-muted/30">
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  {areas.map(area => <button key={area} onClick={() => setSelectedArea(area)} className={`px-4 py-3 rounded-lg text-sm font-medium transition-all duration-300 ${selectedArea === area ? "bg-primary text-primary-foreground shadow-md" : "bg-background border border-border text-foreground hover:border-primary/50 hover:bg-primary/5"}`}>
                      {area}
                    </button>)}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {bedrooms.map(bed => <button key={bed} onClick={() => setSelectedBedrooms(bed)} className={`flex-1 px-4 py-3 rounded-lg text-sm font-medium transition-all duration-300 ${selectedBedrooms === bed ? "bg-primary text-primary-foreground shadow-md" : "bg-background border border-border text-foreground hover:border-primary/50 hover:bg-primary/5"}`}>
            ...
            <div className="p-6 md:p-8 bg-gradient-to-br from-primary/5 via-transparent to-accent/5">
              <div className="grid md:grid-cols-2 gap-6">
                ...
                {/* Monthly Income */}
                <motion.div key={`${selectedArea}-${selectedBedrooms}-monthly`} whileHover={{
                scale: 1.03,
                ...
                {/* Annual Income */}
                <motion.div key={`${selectedArea}-${selectedBedrooms}-annual`} whileHover={{
                scale: 1.03,
                boxShadow: "0 20px 40px rgba(0,0,0,0.15)"
              }} initial={{
                opacity: 0,
