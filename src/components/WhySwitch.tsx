  ...
  return (
    <section className="section-padding bg-accent">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <span className="inline-block px-4 py-2 border border-accent-foreground/30 text-accent-foreground/80 text-xs font-heading tracking-widest uppercase mb-8">
            The Smart Choice
          </span>
          <h2 className="font-heading text-3xl md:text-4xl lg:text-5xl font-bold text-accent-foreground mb-6">
            Why Switch to Short Lets?
          </h2>
          <p className="text-lg text-accent-foreground/70 max-w-3xl mx-auto leading-relaxed">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl mx-auto">
          {reasons.map((reason, index) => (
            <motion.div 
              ...
              className="bg-accent-foreground/5 border border-accent-foreground/10 p-8 text-center hover:bg-accent-foreground/10 transition-all duration-300"
            >
              <div className="inline-flex items-center justify-center w-16 h-16 border-2 border-accent-foreground/30 mb-6">
                <reason.icon className="w-8 h-8 text-accent-foreground" />
              </div>
              <h3 className="font-heading text-sm font-bold text-accent-foreground mb-3 uppercase tracking-wide">
                {reason.title}
              </h3>
